...
For multiple regression model we built a model using our poor quality index as a response variable and household income(hhinc), year, Monthly contract rent(X_30a), and Out of pocket rent (X_31b) as the  explanatory variables.

```{r}
#Preprocess the data
data_part2 <- read.csv("all_data.csv", stringsAsFactors = F)
...
data_part2$X_31b[data_part2$X_31b == 99999] <- NA # Not applicable (occupied rent free or owner occupied)
data_part2$X_31b[data_part2$X_31b == 99998] <- NA # Not reported
```

```{r}
my.reg <- lm(pqi ~ hhinc + year + X_30a, data = data_part2)
summary(my.reg)
```
...
```{r}
my.logreg <- glm(X_25c ~ hhinc + X_30a + X_31b, data = data_part2)
summary(my.logreg)
```
