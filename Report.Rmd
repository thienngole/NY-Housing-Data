---
title: "Indexing NYC Hosuing Quality"
author: "Ahern Nelson"
date: "April 2, 2019"
output:
...
# Introduction
In this paper we look at the NYCHVS data and attempt to a build an index measuring poor housing quality.
With 
# Methodology
The index measures weighted sums of interactions between variables that the authors chose. 
The selected variables were chosen if the authors agreed they describe housing conditions.
however the index is certainly not exhaustive, 
and potentially more data could be collected to better suit this purpose.
# The Index
...
all_data %>% group_by(year, pqi) %>% summarise(count = n()) %>% 
  ...
  ggtitle("Index Percent Frequency Distribution 1991-2017") +
...
all_data %>% group_by(borough, year) %>%
  ...
  scale_x_continuous(breaks = c(1991,seq(1993,2017, 3))) + theme_bw()
```
```{r}
