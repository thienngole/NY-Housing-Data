---
title: "Project Report: Indexing NYC Hosuing Quality - Part 2"
...
k_means_data <- na.omit(scale(k_means_data))
set.seed(25)
...
```{r}
k_means_data$cluster <- arr_clust$cluster
by_clust <- group_by(.data = k_means_data, cluster)
num_of_each_cluster <- summarise(.data = by_clust, num_cluster = n())
num_of_each_cluster
```
This confirms how poorly kmeans, and clustering performs on the data at a unit level. We have a whopping 214 clusters of size, and only one cluster of size 3. This just reaffirms some of the comments made above. Further analysis is needed and a better data aggregation needs to be implemented. It would be interesting to aggregate by subborough and perfor kmeans with 5 clusters to explore how "similar" subboroghs are within the same borough.
